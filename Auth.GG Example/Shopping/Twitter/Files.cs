		...
		public static void ComboProxy()
		{
			using (OpenFileDialog openFileDialog = new OpenFileDialog())
			{
				...
				foreach (string proxyAddress in File.ReadAllLines(openFileDialog.FileName))
				{
					...
					if (flag)
					{
						twitter1check.ProxyList.Add(HttpProxyClient.Parse(proxyAddress));
					}
					else
					{
						...
						if (flag2)
						{
							twitter1check.ProxyList.Add(Socks4ProxyClient.Parse(proxyAddress));
						}
						else
						{
							...
							if (flag3)
							{
								...
								twitter1check.ProxyList.Add(Socks5ProxyClient.Parse(proxyAddress));
							}
							else
							{
			...
			twitter1check.ComboLines = twitter1check.ComboList.Count<string>();
		}
