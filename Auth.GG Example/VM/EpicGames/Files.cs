...
namespace Zuttpal
{
	...
	internal class epicgamesvmfiles
	{
		...
		public static void ComboProxy()
		{
			using (OpenFileDialog openFileDialog = new OpenFileDialog())
			{
				...
				foreach (string proxyAddress in File.ReadAllLines(openFileDialog.FileName))
				{
					...
					if (flag)
					{
						epicgamesvmcheck.ProxyList.Add(HttpProxyClient.Parse(proxyAddress));
					}
					else
					{
						...
						if (flag2)
						{
							epicgamesvmcheck.ProxyList.Add(Socks4ProxyClient.Parse(proxyAddress));
						}
						else
						{
							...
							if (flag3)
							{
								...
								epicgamesvmcheck.ProxyList.Add(Socks5ProxyClient.Parse(proxyAddress));
							}
							else
							{
			...
			epicgamesvmcheck.ComboLines = epicgamesvmcheck.ComboList.Count<string>();
		}
