		...
		public static void ComboProxy()
		{
			using (OpenFileDialog openFileDialog = new OpenFileDialog())
			{
				...
				foreach (string proxyAddress in File.ReadAllLines(openFileDialog.FileName))
				{
					...
					if (flag)
					{
						viaplay1check.ProxyList.Add(HttpProxyClient.Parse(proxyAddress));
					}
					else
					{
						...
						if (flag2)
						{
							viaplay1check.ProxyList.Add(Socks4ProxyClient.Parse(proxyAddress));
						}
						else
						{
							...
							if (flag3)
							{
								...
								viaplay1check.ProxyList.Add(Socks5ProxyClient.Parse(proxyAddress));
							}
							else
							{
			...
			viaplay1check.ComboLines = viaplay1check.ComboList.Count<string>();
		}

		public static void apiprox()
		{
			using (OpenFileDialog openFileDialog = new OpenFileDialog())
			{

				foreach (string proxyAddress in File.ReadAllLines("apiprox.txt"))
				{
					bool flag = Checking.proxytype == 1;
					if (flag)
					{
