		...
		public static void ComboProxy()
		{
			using (OpenFileDialog openFileDialog = new OpenFileDialog())
			{
				...
				foreach (string proxyAddress in File.ReadAllLines(openFileDialog.FileName))
				{
					...
					if (flag)
					{
						Yahoo2check.ProxyList.Add(HttpProxyClient.Parse(proxyAddress));
					}
					else
					{
						...
						if (flag2)
						{
							Yahoo2check.ProxyList.Add(Socks4ProxyClient.Parse(proxyAddress));
						}
						else
						{
							...
							if (flag3)
							{
								...
								Yahoo2check.ProxyList.Add(Socks5ProxyClient.Parse(proxyAddress));
							}
							else
							{
			...
			Yahoo2check.ComboLines = Yahoo2check.ComboList.Count<string>();
		}
