...
namespace Zuttpal
{
	...
	internal class Hotmailinbox1files
	{
		...
		public static void ComboProxy()
		{
			using (OpenFileDialog openFileDialog = new OpenFileDialog())
			{
				...
				foreach (string proxyAddress in File.ReadAllLines(openFileDialog.FileName))
				{
					...
					if (flag)
					{
						Hotmailinbox1check.ProxyList.Add(HttpProxyClient.Parse(proxyAddress));
					}
					else
					{
						...
						if (flag2)
						{
							Hotmailinbox1check.ProxyList.Add(Socks4ProxyClient.Parse(proxyAddress));
						}
						else
						{
							...
							if (flag3)
							{
								...
								Hotmailinbox1check.ProxyList.Add(Socks5ProxyClient.Parse(proxyAddress));
							}
							else
							{
			...
			Hotmailinbox1check.ComboLines = Hotmailinbox1check.ComboList.Count<string>();
		}
